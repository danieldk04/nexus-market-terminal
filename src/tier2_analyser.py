...
def build_bull_bear_prompt(c: dict, sentiment_text: str, filing_ctx: str) -> str:
    ...
    return (
        ...
        f"SOCIAL SENTIMENT & NIEUWS (StockTwits crowd-tags, Reddit, breed nieuws):\n{sentiment_text}"
        f"{filing_block}\n\n"
        ...
        f"## CROWD SENTIMENT CHECK\n"
        f"[Wat zegt de StockTwits/Reddit-crowd? Is dit een contrair signaal (retail te euforisch/pessimistisch "
        f"bij een verder sterke/zwakke fundamentele case) of bevestigt het de fundamentele these? "
        f"Weeg dit licht mee — crowd-sentiment is een tell, geen bewijs.]\n\n"
    ...
    for c in candidates[:TOP_N_ANALYSE]:
        ...
        try:
            ...
            c["tier2"] = {
                ...
                "bluesky_bullish_ratio":    sentiment_ctx.get("bluesky", {}).get("bullish_ratio"),
                "combined_bull_ratio":      sentiment_ctx.get("combined_bull_ratio"),
                "reddit_mentions":          sentiment_ctx["reddit_mention_count"],
                "news_count":               sentiment_ctx.get("news_count", 0),
            }
            updated = True
